...
\todo{generalize for cyclic surfaces}
...
\todo{Why? Somehow the uniqueness of the Schramm pattern has to come in}

\begin{conjecture}[Necessary conditions for the existence of a solution]
...
In this section we describe how to use discrete conformal maps to find analog constructions for discrete surfaces, see, e.g., Figure~\ref{fig:intro_uniformization}, middle.
Section~\ref{sec:tori_general} we show how to find conformally equivalent flat tori and their corresponding lattice for a given polyhedral surface.
Section~\ref{sec:tori:schottky} introduces the discrete uniformization of tori given by Schottky data, i.e., a group of loxodromic transformations together with a set of circles on $\Chat$. 
...
In the last Section~\ref{sec:wente} we investigate the behavior of discrete conformal maps on the discretization of a conformal immersion of the Wente torus.

\subsection{Uniformization with euclidean functional}
\label{sec:tori_general}
...
\begin{proposition}
The polyhedral surface $(\Sigma, \tilde\ell)$ can be laid out to the plane with translational periods.
\end{proposition}
\begin{proof}
...
\label{sec:tori:schottky}

\todo{Include the explicit example from Boris.}
