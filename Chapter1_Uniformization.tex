...
\begin{eqnarray}
	E^{\rm euc,hyp,sph}(\tilde\lambda) &:=& \sum_{\it ijk\in F}\left(f^{\rm euc,hyp,sph}_{\it ijk}(\tilde\lambda) - \frac{\pi}{2}\left(\tilde \lambda_{jk} + \tilde \lambda_{ki} + \tilde \lambda_{ij}\right)\right) + \sum_{i\in V} \Theta_i u_i.
\end{eqnarray}
\end{definition}

The function $f^{\rm euc,hyp,sph}_{\it ijk}$ is different for each of the geometries and defined as
\begin{eqnarray}
f^{\rm euc,hyp,sph}_{\it ijk}(\tilde\lambda) &:=&\betat_{\it jk}^i \tilde \lambda_{jk} + \betat_{\it ki}^j \tilde \lambda_{ki} + \betat_{\it ij}^k \tilde \lambda_{ij}\\ 		
