...
\includegraphics[width=10cm, draft=true]{schottky_g3/genus3_domain2.pdf}
}
...
\includegraphics[height=5cm, draft=true]{circle_domain_hyperbolic/brezel_three_holes_domain.pdf}
}
...
Firstly, we introduce fundamental domains specially designed to match the hyperelliptic structure of the surfaces, i.e., oppositely identified polygons with a branch point at the vertex.
These objects are the natural generalizations of the euclidean lattices in the elliptic case, see Section~\ref{sec:tori}.
Secondly, we conjecture a characterization of hyperelliptic Riemann surfaces in terms of the geometry of the fundamental group. 
...
\begin{conjecture}
Let $R$ be a Riemann surface and let $G$ be its uniformizing group. Let $A_1,\ldots,A_g$ be generators of $G$ with the relation $A_1A_2\ldots A'_1A'_2\ldots=1$. Then the following statements are equivalent:
\begin{compactitem}
\item $R$ is hyperelliptic.
\item The axes of $A_i$ $i=1\ldots g$ meet in a common point $p_0$.
\end{compactitem}
\end{conjecture}
Note that the point $p_0$ is a branch point of the corresponding hyperelliptic algebraic curve.
...
\subsection{Mapping an embedded hyperelliptic surface to $\Chat$}
